...
describe('remark-contentful-image', () => {
  test('contentful のドメインの画像なら、srcset を追加する', async () => {
    const { value } = await processor.process(`![test](https://images.ctfassets.net/123/456/789.jpg)`)
  ...
  test('contentful のドメインの画像でないなら、srcset を追加しない', async () => {
    const { value } = await processor.process(`![test](https://example.com/123/456/789.jpg)`)
    console.log(value.toString())
    expect(value.toString()).toBe(`<p><img src="https://example.com/123/456/789.jpg" alt="test" loading="lazy" /></p>`)
  })
})
