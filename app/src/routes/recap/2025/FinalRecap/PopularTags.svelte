<script lang="ts">
  ...
  let tagInView = $state<boolean[]>(new Array(recap2025.topTags.length).fill(false));
...
<div class="mx-auto flex max-w-6xl flex-wrap justify-center gap-3 md:gap-4 px-4">
  {#each recap2025.topTags as tag, i}
    {@const scale = getTagScale(tag.count)}
    {@const sizeClass = scale > 0.66 ? 'text-2xl md:text-3xl' : scale > 0.33 ? 'text-xl md:text-2xl' : 'text-lg md:text-xl'}
    {@const paddingClass = scale > 0.66 ? 'px-6 py-3 md:px-8 md:py-4' : scale > 0.33 ? 'px-5 py-2.5 md:px-7 md:py-3.5' : 'px-4 py-2 md:px-6 md:py-3'}

    <div
      use:inview
      on:inview_change={(e) => { tagInView[i] = e.detail.inView; }}
