...
<div
  ...
  class="relative overflow-hidden min-h-screen w-full
         bg-gradient-to-br {gradient.bg.from} {gradient.bg.via} {gradient.bg
    .to}"
>
  <!-- Decorative blur orbs -->
  <div
    class="absolute top-0 left-0 w-72 h-72 md:w-96 md:h-96 rounded-full
              bg-gradient-to-br {gradient.orb1.from} {gradient.orb1.to}
              opacity-10 blur-3xl"
  ></div>
  <div
    class="absolute bottom-0 right-0 w-72 h-72 md:w-96 md:h-96 rounded-full
  ...
  >
    ...
    <h2
      class="text-5xl md:text-6xl lg:text-8xl font-black mb-8 md:mb-10
               bg-gradient-to-r {gradient.title.from} {gradient.title
    ...
    >
      今年は AI
      エージェント時代の幕開けとともに、新たな技術探求の旅が始まりました。 78
      本の記事、9
    ...
    <div class="flex flex-wrap justify-center gap-4 md:gap-6 px-4">
      ...
      >
        <div
          class="absolute inset-0 bg-gradient-to-r {gradient.ctaPrimary
            .from} {gradient.ctaPrimary.to}
                    rounded-xl group-hover:shadow-2xl transition-shadow duration-300"
        ></div>
        <div
          class="absolute inset-0 bg-gradient-to-r {gradient.ctaPrimary
            .from} {gradient.ctaPrimary.to}
