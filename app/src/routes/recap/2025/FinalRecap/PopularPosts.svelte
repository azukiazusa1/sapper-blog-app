<script lang="ts">
  ...
  let postInView = $state<boolean[]>(new Array(recap2025.popularPosts.length).fill(false));
...
<div class="mx-auto max-w-4xl space-y-8 px-4">
  {#each recap2025.popularPosts as post, i}
    <a
      href={post.url}
      use:inview
      on:inview_change={(e) => { postInView[i] = e.detail.inView; }}
    ...
    >
      ...
      <div class="bg-gradient-to-r {gradient.border.from} {gradient.border.to} p-[3px] rounded-2xl
                  transform transition-all duration-300
                  group-hover:-translate-y-2 group-hover:shadow-2xl">

        <div class="bg-white rounded-2xl p-6 md:p-8 flex flex-col sm:flex-row gap-6">
          <!-- Gradient rank badge -->
          <div class="flex-shrink-0">
            <div class="relative">
              <div class="bg-gradient-to-br {gradient.badge.from} {gradient.badge.to}
              ...
              <!-- Glow effect -->
              <div class="absolute inset-0 bg-gradient-to-br {gradient.badgeGlow.from} {gradient.badgeGlow.to}
          ...
          <div class="flex-1">
            ...
            <h3 class="text-xl md:text-2xl font-bold mb-4 text-gray-900
                       transition-all duration-300
                       group-hover:bg-gradient-to-r {gradient.titleHover.from} {gradient.titleHover.to}
            ...
            <div class="flex items-baseline gap-2">
              ...
              <span class="text-sm uppercase tracking-wider text-gray-500 font-semibold">views</span>
            </div>
          </div>
        </div>
      </div>
