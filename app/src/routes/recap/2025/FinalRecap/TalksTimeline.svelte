<script lang="ts">
  ...
  let talkInView = $state<boolean[]>(
    new Array(recap2025.talks.length).fill(false),
  );
...
<div class="relative max-w-4xl mx-auto px-4">
  ...
  <div
    class="absolute left-1/2 top-0 bottom-0 w-1
              bg-gradient-to-b {gradient.line.from} {gradient.line
      .via} {gradient.line.to}
      ...
      >
        ...
        >
          ...
          <div class="w-[calc(50%-2rem)]">
            ...
            <div
              class="bg-gradient-to-br {gradient.border.from} {gradient.border
                .to} p-[2px] rounded-xl
                        transform transition-all duration-300 hover:shadow-2xl"
            >
              <div class="bg-white rounded-xl p-5 md:p-6">
                <div
                  class="text-xs font-bold uppercase tracking-wider {gradient.date} mb-2"
                >
                  {talk.eventDate}
                </div>
                ...
                <a
                  ...
                  class="text-sm font-semibold bg-gradient-to-r {gradient.link
                    .from} {gradient.link.to}
        ...
        <div
          class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 hidden md:block z-10"
        >
          <div class="relative">
            <div
              class="w-6 h-6 rounded-full bg-gradient-to-br {gradient.node
                .from} {gradient.node.to}
                        border-4 border-white shadow-lg"
            ></div>
            <!-- Pulse animation -->
            <div
              class="absolute inset-0 w-6 h-6 rounded-full {gradient.pulse} animate-ping opacity-20"
            ></div>
          </div>
        </div>
      </div>
