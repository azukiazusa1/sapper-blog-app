...
<div
  class="mx-auto w-full max-w-[600px] overflow-hidden rounded-lg border border-stone-200 bg-stone-50 dark:border-stone-700 dark:bg-stone-950"
>
  {#each items as item, itemIndex (item.id)}
    {@const expanded = currentExpandedId === item.id}
    <article
      class:border-t={itemIndex !== 0}
      class="overflow-hidden border-stone-200 transition-colors dark:border-stone-700"
    >
      <div class="flex gap-4 px-4 py-5 sm:px-5">
        <div class="flex w-12 shrink-0 flex-col items-center">
          <img
            src={avatarImage}
            alt="azukiazusa"
            class="mt-1 h-10 w-10 rounded-full object-cover border border-stone-200 dark:border-stone-700"
            width="40"
            height="40"
            loading="lazy"
            decoding="async"
        ...
        <div class="min-w-0 flex-1">
            ...
            <div class="min-w-0">
              ...
              <h2 class="mt-1 min-w-0">
                <a
                  ...
                  class="text-lg font-semibold tracking-tight text-stone-900 transition-colors hover:underline dark:text-stone-50"
          ...
          >
            <div
              class="short-thread-content prose prose-zinc max-w-none break-words text-[15px] leading-relaxed dark:prose-invert"
            >
          ...
          <div
            class="mt-4 flex items-center gap-3 text-xs font-medium text-stone-500 dark:text-stone-400"
          >
            <button
              type="button"
              class="inline-flex items-center gap-2 rounded border border-stone-300 px-3 py-1.5 transition hover:bg-stone-100 dark:border-stone-700 dark:hover:bg-stone-900"
              aria-controls={"thread-" + item.id}
              aria-expanded={expanded}
              onclick={() => toggleThread(item.id)}
            >
          ...
          {#if expanded && item.threadCount > 1}
            <div id={"thread-" + item.id} class="mt-4">
              {#each item.htmlThreadItems.slice(1) as threadItem}
                <div
                  class="relative min-w-0 border-t py-4 border-stone-200 dark:border-stone-700"
                >
                  <div
                  ...
                  <div class="min-w-0">
                    ...
                    <div
                      class="short-thread-content prose prose-zinc mt-2 max-w-none break-words text-[15px] leading-relaxed dark:prose-invert"
                    >
                      {@html threadItem}
                    </div>
                  </div>
