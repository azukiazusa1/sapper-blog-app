...
Rust「手続き型」「オブジェクト指向型」「関数型」などさまざまなパラダイムを言語思想に取り入れていますが、とりわけ Haskell や scala などの関数型言語の影響を強く受けております。
...
`Result` 型はエラーになる可能性があることを表す値です。TypeScript でパターンマッチングを用いてエラーハンドリングを実施しようとするとどうしても言語仕様として　`try/catch` が入ってしまう危険性があったのですが、言語仕様として使えるのは嬉しいですね。
