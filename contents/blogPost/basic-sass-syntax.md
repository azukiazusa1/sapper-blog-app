...
Sassには、最初に作られた「SASS記法」と「SCSS記法」があります。
SASS記法は、セレクタの後の`{}`の代わりにインデントを使用する、値の後の`;`セミコロンを省略できるといった、簡素化された記法が特徴です。
一方のSCSS記法はCSSとの互換性を高めた記法です。
...
現在ではSCSS記法が好まれて使用されています。そのため、この記事ではSCSS記法を中心に進めていきます。
なお、動作確認はSassMeisterで行っています。

https://www.sassmeister.com/

# ネスト

Sassの中でも最も使われる機能の1つがネストです。
HTMLの構造のように入れ子で書くことができます。実際に例を見てみましょう。
...
`&`を使用すると、ネストの中で親セレクタを参照することができます。
例えば、次のようにBEMライクな記法をしたいときに重宝します。
...
例えば、HTML内で`nav__item--active`というクラスが付与されているのを発見してそのクラス名でエディタ検索をかけてもヒットすることはありません。
...
[CSSのカスタムプロパティ](https://developer.mozilla.org/ja/docs/Web/CSS/Using_CSS_custom_properties)とは異なる文法でSassの変数を使うことができます。
...
このような場合には、インターポレーション（補完）を使うことで解決します。参照死体変数名を`#{}`で囲って書きます。
...
数値に対して加算`+`・減算`-`・乗算`*`・除算`/`・`剰余`%`を用いて計算をすることができます。
...
除算については、fontプロパティなどの指定で`/`が使われているため演算として機能させるために`()`で加工必要があります。
また、加算では数値の計算だけでなく文字列の結合に使用することもできます。

ところで、CSSには`calc`という同じように演算ができる関数があります。
Sassの演算と`calc`の相違点は以下の通りです。
...
- CSS
 - ブラウザで動的に計算される
 - Sassにはできない単位の違う計算ができる

# @extend

`@extend`を用いると、指定したセレクタのスタイルを継承することができます。
...
`@mixin`を引数を使って利用することで、より柔軟にスタイルを適用することができます。
...
ところで、CSSのプロパティにはカンマ`,`を使うものがありますが、これを値として利用すると引数の区切りと認識されてエラーとなってしまうことがあります。
...
もう一つの方法として、引数の数を固定しないように可変長引数として`@mixin`を定義するという方法があります。
...
可変長引数は、`@include`する際にも使用することができます。
