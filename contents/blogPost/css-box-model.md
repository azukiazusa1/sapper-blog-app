...
`.box1` には `width: 70%` を設定し `.box2` には `width: 30%` を設定しているので、2つの要素を合わせてぴったり `100%` になるはずです。しかし、実際に表示されるレイアウトは以下のように想定とはことなる表示となってしまいます。
...
ここで問題です。ヘッダー要素とメインコンテンツ要素の間は何px間が空いているでしょうか？
