...
また既存のコードベースのアーキテクチャを理解して、コードの設計に沿ったものを生成するといったことはまだできなかったので、コードの品質についてまだまだ不満点があったかと思います。
-->
...
これまでの開発では、小さなバグや機能修正が報告されても、優先度の高いタスクが先行され、修正が後回しになることが多くありました。

エージェントを活用することで、こうした小さなタスクを即座に処理できるようになり、開発者は重要なタスクに集中して取り組むことができるようになります。これにより、開発全体の生産性向上が期待できます。
-->
...
また、プロジェクトのコーディング規約を自作 Lint ルールとして作成するのも有効です。これにより、AI エージェントはプロジェクト固有のコーディング規約を理解し、適切なコードを生成することができます。
-->
...
コーディングエージェントがコード生成の大部分を担うようになると、開発者の主な仕事の一つとして「コードレビュー」の比重が格段に高まると予想されます。特に自律型のエージェントを使用している場合には、途中で介入することはほとんどないため、コードレビューの時間がAIにフィードバックを与える唯一の機会となりえます。
...
<!--
コーディングエージェントに期待しすぎないことが重要です。

何も介入せずに完璧なコードが仕上がることや、複雑な要求を一発で正確に実装してくれることを期待してはいけません。
...
<!--
これは「机に5分座る」効果と同じで、作業興奮（Arbeitsrausch）という心理学的現象に基づいています。

作業興奮とは、ドイツの心理学者クレペリンが提唱した概念で、作業を始めると脳が興奮状態になり、やる気が自然と湧いてくる現象です。「とりあえず机に5分座る」というテクニックは、この作業興奮を意図的に引き起こす方法です。つまり、「やる気が出るから行動する」のではなく、「行動するからやる気が出る」ということです。

コーディングでも同じで、「とりあえずAIに骨格コードを生成してもらう」ことで、動くコードが目の前に現れると「もう少し改善しよう」という気持ちが自然に湧いてきます。これがコーディング版の作業興奮です。

心理的効果として、白紙恐怖症の解消、完璧主義からの解放、作業開始のきっかけ作り、アイデア創出の触媒効果があります。
-->
