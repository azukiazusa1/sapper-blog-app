...
- **A2UI** (Google) — *プロトコル*。何を、どんな JSON で送るかの仕様
- **json-render** (Vercel Labs) — *フレームワーク*。それを実装/描画する側のライブラリ (A2UI もサポート)
...
ここで重要なのが、紹介する 2 つはレイヤーが違うという点です。Google の A2UI は「Agent と UI の間の通信プロトコル」、つまり MCP と同じレイヤーの仕様です。一方、Vercel Labs の json-render は「そのプロトコルを受け取って実際に描画する側のフレームワーク」で、json-render は A2UI の実装もサポートしています。順番に見ていきます。
-->
...
**Agent と UI の間の通信プロトコル**。実装ではなく仕様。
...
→ "How can AI agents safely send rich UIs across trust boundaries?" への **プロトコル層の答え**
...
**実装側のフレームワーク**。プロトコルではなくライブラリ。

- A2UI のような JSON spec を、**React / Vue / Svelte などの実部品に解釈・描画する**側
- 開発者が **コンポーネントカタログ** (Zod スキーマ) を定義して AI を制約する
- **A2UI の実装もサポート** — プロトコルとフレームワークが噛み合う関係

3 ステップの仕組み

1. 開発者が **カタログ** (Zod スキーマ) を定義
2. AI が **カタログ準拠の JSON** をストリーミング生成
3. json-render が **React コンポーネント** として描画

→ "guardrails through constraints"。AI は決められた部品しか出力できない

<!--
次に実装側、json-render です。Vercel Labs が公開しているフレームワーク、つまりライブラリです。先ほどの A2UI のような JSON spec を受け取って、React や Vue や Svelte の実コンポーネントに解釈して描画する役割を担います。
ポイントは、開発者があらかじめコンポーネントのカタログを Zod で定義して、AI はそのカタログにあるものしか使えないようにすることです。guardrails through constraints と表現されていて、AI に自由に HTML を出させるのではなく、決められた部品の組み合わせだけを許す、という設計思想ですね。
そして重要なのが、json-render は A2UI の実装もサポートしている点です。つまり A2UI というプロトコルを実際に React アプリで動かすときの受け皿の一つとして使える、ということになります。プロトコル仕様とフレームワーク実装、それぞれが別々のレイヤーで噛み合うことで、エコシステムが立ち上がっていく構図です。
-->
...
| レイヤー                | 役割                                  | 代表例                                    |
| ----------------------- | ------------------------------------- | ----------------------------------------- |
| **プロトコル** (HTML 系) | Agent が UI を返す仕様 (HTML を運ぶ)  | **MCP Apps** (Anthropic)                  |
| **プロトコル** (JSON 系) | Agent が UI を返す仕様 (JSON で記述)  | **A2UI** (Google)                         |
| **フレームワーク**      | プロトコルを受けてネイティブ部品で描画 | **json-render** (Vercel) — A2UI 対応      |
...
<!--
最後にレイヤー別に整理します。MCP Apps と A2UI は同じプロトコル層、つまり Agent が UI を返すときの「仕様」です。違いは、MCP Apps が HTML をそのまま運ぶのに対して、A2UI は JSON で UI を宣言する、という点ですね。
そして json-render はその下のフレームワーク層、つまりプロトコルを受けて実際にネイティブ部品で描画する側です。json-render は A2UI に対応しているので、A2UI プロトコルを採用したアプリでは json-render が描画エンジンとして使える、という関係になります。
プロトコルとフレームワーク、レイヤーが違うものを並べて比較してしまいがちですが、実際には別のレイヤーで噛み合って一つのスタックを作っている、というのが正しい理解です。
