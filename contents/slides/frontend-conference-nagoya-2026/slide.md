...
- カタログと仕様を AI のシステムプロンプトとして与えるて JSON を生成させる
...
続いて AI が実際に生成する JSON です。AI には json-render の仕様とカタログをシステムプロンプトとして与えて、JSON を生成させます。
この JSON を見ればわかるように、AI はカタログに完全に拘束されます。
-->
...
![](./images/webmcp-demo.png)

<!--
実行のイメージはこのようになります。「卵を買う予定を追加して」というプロンプトに対して、{ text: "卵を買う" } というツール呼び出しが行われ、TODO アイテムが追加された状態になっています。
 -->
...
- それでも WebMCP が必要なのは、**人間と AI が同じインターフェース上で協業する**から
  - 使いなれたダッシュボードを眺めながら、AI に必要な操作だけを任せたり、データを要約してもらうという流れが自然にできる
...
[AIエージェント時代のWeb〜いま、第二のレスポンシブ設計が始まっている - Nothing ventured, nothing gained.](https://takoratta.hatenablog.com/entry/2026/04/28/112859)
...
- **WebMCP** — Web という既存のインターフェースを人間と AI で共有し、AI は必要な操作だけをツールとして呼び出す
...
WebMCP は、Web という既存のインターフェースを人間と AI で共有し、AI は必要な操作だけをツールとして呼び出すことが目的です、単に AI に自動化させたければ、API を作ればいいわけで、WebMCP は人間と AI の協業を前提とした設計になっています。
-->
